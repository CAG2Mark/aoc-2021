...
def get_moves(burrows, hallway):
    ...
    for i in range(len(hallway)):
        ...
        if i in burrowpos_rev:
            ...
            # if burrow is already complete, ignre
            if len(burrow) == 2 and burrow[0] == pos and burrow[1] == pos: continue
            # if burrow is partially complete, also ignore
            if len(burrow) == 1 and burrow[0] == pos: continue
            if len(burrow) > 2: 
                print("Error: burrow length > 2")
                return
        ...
        for j in range(len(hallway)):
            ...
            if j in burrowpos_rev and len(burrows[burrowpos_rev[j]]) >= 2: continue
            ...
            if j in burrowpos_rev:
                ...
                if bur[burpos] and bur[burpos][0] != temp: continue
                if len(bur[burpos]) >= 2: continue
